////////////////////////////////////////////////////////////////////////////////
//DAQ_FPGA_v2
//Purpose:implementing an additional function in order to replicate the data
//        format described in table 2. Then we had to implement a new method
//        which allowed us to join the independent outputs of the previous step
//        into a single common output. This common output is twice larger than
...
//thdata pci = {4, &data_write[0]}; //ask
//thdata fpga = {5, &empty[0]};
//thdata eb = {6, &empty1[0]};
int thread_args[4];
...
//pthread_create(&threads[2], NULL, (void *) &write_func,(void *) &pci);
//pthread_create(&threads[3], NULL, (void *) &read_func, (void *) &fpga);
//pthread_create(&threads[4], NULL, (void *) &read_func, (void *) &eb);
...
for(int slots = 0; slots < slots_to_be_written; ++slots){
  ...
  //if ((x&0xf0000000)!= 0) then it is the header
...
diff1_s1=0;   //problem with 00000000 is happening here
...
//printf ("xd");
//dum = -2;
//ret = write(fd2, &dum,4);  //change this fffffff
